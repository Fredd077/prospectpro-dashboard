...
const TAB_CLASSES = {
  active: 'px-4 py-3 text-sm font-medium border-b-2 border-primary text-primary',
  inactive: 'px-4 py-3 text-sm font-medium border-b-2 border-transparent text-muted-foreground hover:text-foreground',
}
...
export default async function PipelinePage({ searchParams }: PageProps) {
  ...
  const activeTab = params.tab === 'analisis' ? 'analisis' : 'pipeline'
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center border-b border-border bg-background px-8 shrink-0">
          <Link
            ...
            className={activeTab === 'pipeline' ? TAB_CLASSES.active : TAB_CLASSES.inactive}
          >
            Kanban
          </Link>
          <Link
            href={buildUrl({ period, refDate: refDateParam || undefined, tab: 'analisis' })}
            className={activeTab === 'analisis' ? TAB_CLASSES.active : TAB_CLASSES.inactive}
          >
            Análisis
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap border-b border-border bg-background px-8 py-3 shrink-0">
          <div className="flex items-center rounded-md border border-border overflow-hidden">
            {PERIOD_OPTIONS.map(({ value, label }) => (
              <Link key={value}
                href={buildUrl({ period: value, refDate: refDateParam || undefined, tab: activeTab === 'analisis' ? 'analisis' : undefined })}
        ...
        {/* ── Análisis tab: vertical scroll, no horizontal overflow needed ── */}
        {activeTab === 'analisis' && (
