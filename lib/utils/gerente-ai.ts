...
import { startOfWeek, endOfWeek, addWeeks, parseISO, format, startOfMonth, endOfMonth, startOfYear, endOfYear } from 'date-fns'
...
/** Compute the Monday-to-Sunday week boundaries covering [startISO, endISO]. */
function buildWeeks(startISO: string, endISO: string) {
  ...
  while (cursor <= rangeEnd) {
    ...
    weeks.push({
      ...
      label: format(cursor, weeks.length === 0 || cursor.getDate() <= 7 ? 'd MMM' : 'd MMM', { locale: es }),
    })
...
/** Preset date ranges for the filter. Today = 2026-04-26 */
export function presetRange(preset: PeriodPreset): { start: string; end: string } {
  ...
  if (preset === 'year') {
    return { start: toISO(startOfYear(now)), end: toISO(endOfYear(now)) }
  }
  // custom: default to current month
  return { start: toISO(startOfMonth(now)), end: toISO(endOfMonth(now)) }
}

const EMPTY: GerenteAnalytics = {
  weekLabels: [],
  startISO: '',
  endISO: '',
  reps: [],
...
export async function fetchGerenteAnalytics(
  ...
  if (userIds.length === 0) return { ...EMPTY, startISO, endISO }

  const weeks = buildWeeks(startISO, endISO)
  if (weeks.length === 0) return { ...EMPTY, startISO, endISO }
  ...
  const [profilesRes, activitiesRes, logsRes] = await Promise.all([
    ...
    service.from('activities').select('id,name,type,channel,weekly_goal,user_id').eq('status', 'active').in('user_id', userIds),
    service.from('activity_logs').select('user_id,activity_id,real_executed,log_date')
      .gte('log_date', rangeStart).lte('log_date', rangeEnd).in('user_id', userIds),
  ])
  ...
  const reps: RepAnalytics[] = profiles.map((p) => {
    ...
    const actPerf: ActivityPerformance[] = userActs.map((act) => {
      ...
      const totalGoal = (act.weekly_goal ?? 0) * weeksCount
    ...
    const withGoal    = actPerf.filter((a) => a.totalGoal > 0)
    const bestActivity  = withGoal[0] ?? null
    const worstActivity = withGoal[withGoal.length - 1] ?? null
    ...
    return { userId: p.id, name: p.full_name ?? p.email, email: p.email, weeklyTrend, avgCompliance, activities: actPerf, bestActivity, worstActivity, totalCheckIns, lastCheckIn }
  })
  ...
  const teamTrend = weeks.map(({ start, label }) => {
    ...
    const avgPct = weekPcts.length > 0 ? Math.round(weekPcts.reduce((s, p) => s + p, 0) / weekPcts.length) : 0
  ...
  return {
    ...
    summary: {
      totalReps,
      avgCompliance,
      onTrackCount:        reps.filter((r) => r.avgCompliance >= 70).length,
      atRiskCount:         reps.filter((r) => r.avgCompliance >= 40 && r.avgCompliance < 70).length,
      criticalCount:       reps.filter((r) => r.avgCompliance < 40).length,
...
export function buildGerenteContext(analytics: GerenteAnalytics, company?: string): string {
  const { summary, reps, teamTrend, startISO, endISO } = analytics
  const lines: string[] = [
    `Empresa: ${company ?? 'N/A'}`,
    `Período analizado: ${startISO} al ${endISO} (${analytics.weekLabels.length} semanas)`,
    ``,
    `=== RESUMEN DEL EQUIPO ===`,
